...
// Mesma tela (GR669) dos repos irmaos de BA — checklists-visita-previa (UTD) e
// checklists-lpt (LPT); aqui a unidade e CE e o Tipo de Checklist e o
// "Formulario de Vistoria de Obras". Passos:
...
//   -> Finalidade = "10 - Vistoria de Obras Eletrica"
//   -> Tipo de Checklist = "Formulario de Vistoria de Obras"
...
// colapsados. Necessario porque o texto exato do GPM ("Formulário de Vistoria
// de Obras") pode
...
// Credenciais: o padrao da unidade CE e GPM_USER/GPM_PASS (mesmos secrets dos
// repos irmaos de CE). GPM_CE_USER/GPM_CE_PASS existem so como override, caso
// um dia este robo precise de um login proprio.
function credenciais() {
  const user = process.env.GPM_CE_USER || process.env.GPM_USER;
  const pass = process.env.GPM_CE_PASS || process.env.GPM_PASS;
  const origem = process.env.GPM_CE_USER ? "GPM_CE_USER/GPM_CE_PASS (override)" : "GPM_USER/GPM_PASS";
...
async function login(page, cfg) {
  ...
  if (!user || !pass) {
    ...
    throw new Error("Tela de login detectada mas faltam GPM_USER/GPM_PASS (ou GPM_CE_USER/GPM_CE_PASS) no ambiente.");
  }
...
async function selecionarChoices(root, cfg, campo, alvo, token, tms = {}) {
  ...
  // Nos repos irmaos de BA o dropdown tinha 5 opcoes contendo "Visita Prévia" e
  // casar por substring exportava o checklist errado sem ninguem perceber. A
  // lista de CE ainda nao foi inspecionada — por isso a regra continua: so o
  // texto EXATO (normalizado) e aceito.
  const bateu = (t) => {
    const tn = norm(t);
  ...
  // seleciona o item destacado, que e o primeiro da lista filtrada — nao
  // necessariamente o alvo (queimou o run 31422888135 no repo UTD). Com o alvo
  // token "Vistoria de Obras" pode casar com varios tipos de CE; o clique por
  // texto exato e o que garante que so o alvo seja pego.
...
// antes de o AJAX responder — a tela parecia ter 0 tipos e a rodada morria
// dizendo que o tipo nao existe (run 35128440048). Placeholder nao conta, nem
// no widget nem no <select> nativo.
...
async function baixarChecklists(page, cfg, mesAno, intervalo) {
  ...
  console.log(`\n=== Checklists Pergunta/Resposta (Formulario de Vistoria de Obras, CE) | ${vInicio} a ${vFim} | arquivo ${mesAno}.csv ===`);
