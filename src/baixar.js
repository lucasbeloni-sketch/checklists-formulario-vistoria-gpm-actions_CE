// Orquestrador: login no GPM CE -> exporta Checklists Pergunta/Resposta
// (Formulario de Vistoria de Obras) do periodo ancorado em ontem (D-1) ->
// extrai o CSV do zip -> envia/sobrescreve mm.aaaa.csv na pasta
// Checklists_Vistoria do Drive.
//
...
(async () => {
  ...
  try {
    ...
    const origemBruta = parseCsv(buffer);
    ...
    // Padroniza no layout da base (layout.json) antes de subir. A pasta e
    // carregada por uma plataforma, entao todo arquivo tem que ter o MESMO
    // cabecalho. Se o GPM ganhar pergunta nova, ela e anexada no fim e o run
    // avisa — nunca descartada.
    //
    // BOOTSTRAP: enquanto layout.colunas estiver vazio (layout ainda nao
    // calibrado pra este formulario) subimos o export COMO VEIO. Isso e proposital: inventar
    // um layout a partir de um unico export descartaria as perguntas que nao
    // aparecem nesse mes. Rode `npm run gerar-layout` depois do backfill.
    let bufferFinal;
    if (!layout.colunas.length) {
      bufferFinal = buffer;
      console.warn(`[run] layout.json ainda VAZIO (bootstrap): subindo o export como veio, ${origemBruta.header.length} colunas. Rode "npm run gerar-layout" quando a pasta tiver os arquivos do backfill.`);
    } else {
      ...
      bufferFinal = Buffer.from(serializeCsv(destino.header, destino.rows), "utf8");
