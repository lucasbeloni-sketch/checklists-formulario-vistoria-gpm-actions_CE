...
test("selecionarChoices escolhe Finalidade pelo token e confirma no select nativo", { skip: !temChromium }, async () => {
  ...
  try {
    ...
    assert.strictEqual(r.text, "10 - Vistoria de Obras Elétrica");
    assert.strictEqual(r.value, "10");
    assert.deepStrictEqual(await lerSelect(page, "#finalidade"), { value: "10", text: "10 - Vistoria de Obras Elétrica" });
  } finally { await browser.close(); }
});
...
test("Tipo de Checklist so e selecionavel depois da Finalidade (AJAX)", { skip: !temChromium }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /nao selecionou "Formulário de Vistoria de Obras"/
    );
...
test("token de busca inteiro seria descartado pelo filtro fuzzy — o curto acha", { skip: !temChromium }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => selecionarChoices(page, CFG, "finalidade", CFG.finalidade, "10 - Vistoria de Obras Eletrica", RAPIDO),
      /nao selecionou/
    );
    const r = await selecionarChoices(page, CFG, "finalidade", CFG.finalidade, CFG.finalidadeSearch);
    assert.strictEqual(r.value, "10");
...
test("tipo: escolhe o alvo e nao os outros com 'Vistoria de Obras'", { skip: !temChromium }, async () => {
  ...
  // alvo, nunca no primeiro da lista filtrada.
  const { browser, page } = await abrir();
  try {
    await selecionarChoices(page, CFG, "finalidade", CFG.finalidade, CFG.finalidadeSearch);
    await esperarTiposCarregar(page, CFG);
...
test("tipo: recusa quando o alvo nao existe na lista (nao aceita parecido)", { skip: !temChromium }, async () => {
  ...
  try {
    ...
    await esperarTiposCarregar(page, CFG);
    await assert.rejects(
      () => selecionarChoices(page, CFG, "tipoChecklist", "XPTO - Vistoria de Obras", "Vistoria", RAPIDO),
      /nao selecionou "XPTO - Vistoria de Obras"/
    );
  } finally { await browser.close(); }
});
