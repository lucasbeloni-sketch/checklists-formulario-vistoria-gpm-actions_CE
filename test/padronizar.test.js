...
// Este formulario tem outras perguntas que os de BA e o CSV do GPM nao tem schema
// fixo, entao este repo NASCE com layout.json vazio e o preenche depois do
// backfill (npm run gerar-layout). O teste cobre os dois estados: vazio =
// bootstrap valido; preenchido = tem que ser coerente (sem repetidas, com as 8
// colunas fixas do relatorio na frente, contagem batendo).
test("layout.json: vazio (bootstrap) ou coerente depois de gerado", () => {
  const c = layoutReal.colunas;
  assert.ok(Array.isArray(c), "colunas tem que ser array");
  if (!c.length) {
    assert.strictEqual(layoutReal.canonicas, 0);
    assert.strictEqual(layoutReal.geradoEm, null, "layout vazio nao tem data de geracao");
    return;
  }
  assert.strictEqual(new Set(c).size, c.length, "nao pode ter nome repetido");
  assert.ok(layoutReal.geradoEm, "layout gerado tem que registrar geradoEm");
  assert.ok(layoutReal.canonicas > 0 && layoutReal.canonicas <= c.length);
  // As 8 primeiras sao as fixas do relatorio (iguais em todos os tipos).
  assert.deepStrictEqual(c.slice(0, 8), [
